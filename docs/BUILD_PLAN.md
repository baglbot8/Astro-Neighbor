...
## Phase 5: The finale, heat wave 1, and dev tools

**Play at the end:** the fifth part → Professor Comet's call → every friend waiting on the Commons → the meeting →
your choice ("Send her" or "Give me a moment") → the empty rocket breaks the giant asteroid into a meteor shower, in
...
(not `space_travel`), which keeps the showpiece near the flight's heat cost; L is split into L1 (the rock) and L2 (the
send-off and shower); dev_menu.gd belongs to a new DEV builder. The heat items come from a measured diagnosis with
adversarial re-measurement (`docs/OPEN_ISSUES.md` 57).

| builder | model (critic) | owns | does |
|---|---|---|---|
| L0: prework | Sonnet + Haiku (Sonnet) | `world.gd` finale hook, `visitor_system.gd` (visits off at stages 1-3), `dialogue_box.gd` + `dialogue_runner.gd` (`arm_delay`), new `campaign/finale_lines.gd` | the seams the rest plug into |
| K: flow and state | Sonnet (Opus) | new `campaign/finale_state.gd`, `campaign/finale.gd` | spec §1, the call, §6, §10 debug API |
| M1: skiff | Opus (Opus) | `rocket/rocket_model.gd`, new `rocket/skiff_mesh_lib.gd` | spec §4 skiff |
| L1: giant asteroid | Opus (Opus) | new `campaign/giant_asteroid.gd` | spec §4 asteroid |
| HEAT-R: render quick wins | Sonnet (Sonnet) | `planet/collectible.gd`, `ui/title/title_screen.gd`, `ui/hud/hud.gd`, `player/camera_rig.gd`, `shaders/grass_planet.gdshader`, `shaders/plaza_tiles.gdshader`, `planet/planet_data.gd` | heat items 1, 2, 3, part of 9 |
| HEAT-WEB: draw gate | Sonnet (Opus) | `project.godot`, new `autoload/draw_gate.gd` | heat item 4 |
| HEAT-LOAD: warm-up | Sonnet (Opus) | `autoload/scene_router.gd`, `world/environment.gd`, new `world/shader_warmup.gd` | heat item 5 |
| HEAT-NPC: idle skip | Sonnet (Sonnet) | `characters/npc.gd` | heat item 6, plus facing and stroll hooks for K2 |
| HOOKS: dev hooks | Sonnet (Sonnet) | `projects/project_system.gd`, `favors/favor_system.gd`, `campaign/visitor_system.gd`, `onboarding/intro_director.gd`, `player/player.gd` | public dev_* hooks; part of heat 9 |
| DEV: dev menu + perf overlay | Sonnet (Sonnet) | `ui/pause/dev_menu.gd`, new `ui/pause/perf_overlay.gd` | tabs for Status, Story, Friends, Games, World, Perf |
| K2: meeting and choice | Opus (Opus) | new `campaign/finale_meeting.gd` | spec §2 meeting and choice |
| M2: pad and flight wiring | Sonnet (Sonnet) | `rocket/rocket_pad.gd`, `rocket/space_travel.gd`, `rocket/journey_state.gd`, `minigames/replay_board.gd`, `minigames/replay_board_panel.gd` | the skiff on every pad and flight; part of heat 9 |
| L2: send-off and shower | Opus (Opus, max) | new `campaign/finale_launch.gd`, `campaign/star_shower.gd` | spec §2 send-off, §4 shower |
| G: gift | Sonnet (Opus) | new `campaign/finale_gift.gd` | spec §2 gift |

Order: L0, M1, L1, the four heat builders and DEV's build start together; K and HOOKS after L0; M2 after M1 passes;
K2 after K, L1, HEAT-NPC and HOOKS pass; L2's build after K2's build report and its critic after K2 passes; DEV's
critic after K, HOOKS, HEAT-WEB and HEAT-LOAD pass; G after L2 and M2 pass; then the integration run. Sounds
(`finale_impact`, `finale_shower`, `finale_hero_star`, `skiff_reveal`) are generated by the lead after the builders
finish. Heat items 7, 8 and 10 wait for numbers from the phone (the dev menu's Perf tab).

...
The standard is **Animal Crossing: New Horizons**. Charming, polished, alive, zero jank. Critics are adversarial: assume the builder cut corners and go find where.
...
4. Compare against `reference/` images. Ask: would this frame look at home in ACNH?
...
The user said: "compare the characters close side by side to animal crossing characters and make sure they're nearly indistinguishable." For ANY character (astronaut, Zorp, Bolt, shopkeepers, mannequins):
1. Capture a close-up (face + full body, front, 3/4, back) and a gameplay-distance shot.
2. Build a side-by-side sheet with `python3 tools/compare.py OUT.png "OURS=<capture>:<crop>" "AC=<reference crop>" ...` using reference/AC Characters.png, "AC Reference 2 copy.jpg" (player), "AC Reference 3 copy.jpg" (villager lineup), "AC Reference 4 copy.jpg" (Rosie).
3. Read the sheet. The verdict question is: *could this be a screenshot from Animal Crossing?* Eye size/shape/placement, head shape, body squatness, matte toon materials, chunky hair, tiny nose/mouth, blush — all must match the AC grammar in docs/STYLE_GUIDE.md "Character rules". If a stranger could tell it apart by style (not by being an astronaut/alien/robot), it FAILS.
...
**Headline gate: `saturation p90 <= 0.68`** on any planet daytime gameplay frame (`tools/palette.py` prints it). Ours was 0.811 against ACNH's 0.673 and the user called the colours "popping too much". Also fail if any dominant swatch is above S 0.60, if luma p95 > 0.93, or if grass/white/sky-blue read as vivid rather than pastel. Natural surfaces must be matte (spec ~0.05); gloss is reserved for the visor, water, chrome and glass. Space scenes are exempt - the user says they already look good.
...
1. Run `python3 tools/palette.py "OURS=<your gameplay capture>" "AC=reference/AC Reference 2 copy.jpg" "AC2=reference/AC Reference 3 copy.jpg"`.
...
3. Run `python3 tools/compare.py` sheets for shapes too, not just characters: our tree next to an AC tree, our cloud next to an AC cloud, our ground next to AC ground. Judge silhouette structure — tiered/flat-based/crisp, not bubbly.
...
- [ ] UI: rounded cream panels, brown text, consistent font, nothing overflows, focus visible, pop animation
...
- [ ] Dialogue typewriter with blips; advancing feels snappy
...
Any placeholder geometry or "TODO" in a shipped path · console errors/warnings · missing showcase scene · builder claims done but a listed feature is absent · text overflow · frame time > 16 ms · player can fall through or leave the planet · anything that would embarrass us next to the reference screenshots.

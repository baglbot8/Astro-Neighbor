...
## Builders
1. Read `docs/ARCHITECTURE.md` (§1.1 says what this game is *not*) and `docs/STYLE_GUIDE.md` fully. Then, with the Read tool, look at the `reference/` images that STYLE_GUIDE's reference table marks as targets (and, for cast or UI work, the ones it marks as shape vocabulary or layout ideas). The table says which is which; the rest of the folder is not for you.
...
4. Iterate with your own eyes: `tools/check.sh`, then `tools/capture.sh showcase/x.tscn x 60` and Read the PNGs. Fix what you see. Repeat until it passes the STYLE_GUIDE gates and the QUALITY_BAR checklists and reads as this game (ARCHITECTURE §1, §1.1).
